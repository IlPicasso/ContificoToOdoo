...
- **1.0.56 (2026-05-06)**: Se corrigieron links de descarga por `run` en frontend/backend: `stock_quant_csv` vuelve a apuntar a `stock_quant.csv` y el endpoint `/odoo-migration/runs/{run_id}/files/{filename}` ahora permite también `odoo_product_templates.csv`, `odoo_variant_sku_mapping.csv`, `odoo_stock_quant.csv`, `odoo_import_validation_report.csv` y `run_summary.json` para evitar errores de descarga 400/404 con enlaces visibles en UI.
