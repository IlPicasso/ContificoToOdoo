...
class OdooMigrationService:
    ...
    def merge_phase2_with_odoo_export(
        ...
        *,
        ...
        def _norm_tmpl_id(tmpl_id: str) -> str:
            return tmpl_id.strip().casefold()

        # Build two lookups from Odoo export:
        #   primary: (norm_tmpl_id, norm_variant_values) — works when Odoo export has __import__.* IDs
        ...
        with odoo_export_csv.open("r", newline="", encoding="utf-8-sig") as f:
            for row in csv.DictReader(f):
                ...
                if not pp_ext_id:
                    continue
                norm_vars = _norm_variant_values(variant_vals)
                if tmpl_id:
                    primary_key = (_norm_tmpl_id(tmpl_id), norm_vars)
                    tmpl_id_lookup[primary_key] = pp_ext_id
                    odoo_pp_to_key[pp_ext_id] = primary_key
                if tmpl_name:
        ...
        with phase2_csv.open("r", newline="", encoding="utf-8-sig") as f:
            for row in csv.DictReader(f):
                ...
                norm_vars = _norm_variant_values(variant_values)

                pp_ext_id = None
                match_method = None

                # Primary: match by (template External ID, variant values)
                if tmpl_id:
                    primary_key = (_norm_tmpl_id(tmpl_id), norm_vars)
                    pp_ext_id = tmpl_id_lookup.get(primary_key)
                    if pp_ext_id:
                        match_method = "tmpl_id"
                        matched_by_tmpl_id += 1

                # Fallback: match by (template name, variant values)
                if pp_ext_id is None and name:
                    fallback_key = (_norm_name(name), norm_vars)
                    pp_ext_id = name_lookup.get(fallback_key)
                    if pp_ext_id:
                        match_method = "name"
                        matched_by_name += 1
                ...
                if pp_ext_id:
                    ...
                    matched_rows.append({"id": pp_ext_id, "match_method": match_method, **row})
                else:
    ...
    # ── New pipeline (Módulo 1 recode) ───────────────────────────────────────

    def export_phase1_new(
        self,
        *,
        ...
        saldos_csv: Path | None = None,
    ) -> dict[str, Any]:
        """New orchestration method using the processing/ pipeline.

        Replaces the old _phase1_prepare_products + _write_variant_import_outputs flow.
        Generates four numbered CSV files ready for Odoo import — no merger step required.
        """
        from .processing.normalizer import parse_sku
        from .processing.grouper import group_products as _group_products
        from .processing.validator import validate as _validate
        from .processing.generators import generate_all as _generate_all, GeneratorOptions

        output_folder.mkdir(parents=True, exist_ok=True)

        # Deduplicate SKUs (first occurrence wins, extras tracked for validation)
        seen_skus: set[str] = set()
        raw_skus: list[str] = []
        ...
        # Resolve category and inject _stock_map + _category into each item
        enriched: list[dict[str, Any]] = []
        for item in deduped:
            sku = str(item.get("codigo") or "").strip()
            name = str(item.get("nombre") or "").strip()
            cat_id = str(item.get("categoria_id") or item.get("categoria") or "")
            category = (
                CONTIFICO_CATEGORY_TO_ODOO.get(cat_id)
                or self._detect_category_from_name(name, cat_id, sku)
            )
            stock_map = self._extract_stock_from_item(item)
            enriched.append({**item, "_category": category, "_stock_map": stock_map})
        ...
        report = _validate(
            groups,
            raw_sku_list=raw_skus,
            saldos_csv=saldos_csv,
            write_to=validation_report_path,
        )
        ...
        return {
            ...
            "total_simples": report.total_simples,
            "total_with_attributes": report.total_with_attributes,
    ...
    def _detect_category_from_name(self, name: str, categoria_id: str, sku: str) -> str:
        """Thin wrapper around rules.detect_category for internal use."""
        from .rules import detect_category
        return detect_category(name, categoria_id, sku=sku)
