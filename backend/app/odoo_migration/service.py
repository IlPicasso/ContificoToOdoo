...
class OdooMigrationService:
    ...
    def generate_products_and_stock_csv(self, *, page_size=200, max_pages=200, progress_callback: Callable[[dict[str, Any]], None] | None = None) -> MigrationOutput:
        ...
        debug_lines=[f'start={datetime.utcnow().isoformat()}Z',f'page_size={page_size}',f'max_pages={max_pages}']; raw_lines=[]
        ...
        for i,item in enumerate(products, start=1):
            sku=str(item.get('codigo') or '').strip(); name=str(item.get('nombre') or ''); barcode=str(item.get('codigo_barra') or '').strip()
            ...
            if should_exclude_zero_stock(stock_total):
        ...
        summary={"total_extracted":len(products),"total_stock_gt_zero":len(products)-len(zrows),"total_excluded_zero_stock":len(zrows),"total_ok":counts['ok'],"total_manual_review":counts['manual_review'],"total_error":counts['error']}
    ...
    def _extract_stock_by_warehouse(item: dict[str, Any]):
        ...
        if isinstance(raw,list):
            for e in raw:
                ...
                code=str(e.get('codigo') or e.get('siglas') or e.get('bodega') or '').upper(); qty=float(e.get('cantidad') or e.get('stock') or 0)
                for key in result:
                    if key in code: result[key]=qty
        return result
