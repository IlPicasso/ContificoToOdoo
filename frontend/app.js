...
// ── Nueva Pipeline (Módulo 1 recode) ────────────────────────────────────────

function setNuevaStatus(msg) { const el = $('nuevaStatus'); if (el) el.textContent = msg || ''; }

function renderNuevaLinks(files) {
  const el = $('nuevaLinks');
  if (!el) return;
  el.innerHTML = '';
...
function renderNuevaValidation(data) {
  const el = $('nuevaValidationPanel');
  if (!el) return;
  const warnings = [];
  if (data.duplicate_skus > 0) warnings.push(`${data.duplicate_skus} SKUs duplicados en Contífico`);
  if (data.barcode_conflicts > 0) warnings.push(`${data.barcode_conflicts} conflictos de barcode`);
  if (data.large_templates > 0) warnings.push(`${data.large_templates} plantillas con >20 variantes`);
  if (!warnings.length) { el.classList.add('hidden'); return; }
  el.innerHTML = `<div class="warning-box"><strong>Advertencias de validación:</strong><ul>${warnings.map(w => `<li>${w}</li>`).join('')}</ul></div>`;
  el.classList.remove('hidden');
}

$('runNuevaPipeline').addEventListener('click', async () => {
  try {
    setNuevaStatus('Procesando...');
    $('runNuevaPipeline').disabled = true;
    const data = await apiUpload('/odoo-migration/products-stock/export-phase1-new', 'nuevaRawFile', {
      include_zero_stock: $('nuevaIncludeZeroStock').checked,
      include_stock: $('nuevaIncludeStock').checked,
    });
    show('nuevaSummaryOut', data);
    renderNuevaValidation(data);
    renderNuevaLinks(data.files || {});
    setNuevaStatus(
      `Listo. ${data.total_templates} plantillas, ${data.total_variants} variantes (${data.total_simples} simples, ${data.total_with_attributes} con atributos).`
    );
  } catch (e) {
    showErr(e);
    setNuevaStatus(`Error: ${e.message}`);
  } finally {
    $('runNuevaPipeline').disabled = false;
  }
});
